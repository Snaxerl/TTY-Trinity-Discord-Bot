    ...
    async await_reactions() {
        ...
        { max: 1, time: 30000 }).then(collected => {
            ...
            if (this.input_answer == this.correct_answer) {
                ...
                this.question_message.edit('Richtig! :smile:')
                this.end_game()
            }
            else {
                this.answer_array[this.input_answer - 1] = this.answer_array[this.input_answer - 1] + ' ❌'
                ...
                .setFooter('Category - ' + atob(this.question.results[0].category) + ', Difficulty - ' + atob(this.question.results[0].difficulty))
                this.question_message.edit(this.question_embed)
                this.question_message.edit('Leider falsch. Die richtige Antwort war ' + this.reactions[this.correct_answer - 1])
                this.end_game()
            }
        }).catch(() => {
            this.question_message.edit('Du hast zu lange gebraucht. Die Antwort war ' +  this.reactions[this.correct_answer - 1])
            this.end_game()
        })
    }
    async end_game() {
